...
export interface ArtSpec { label: string; size: `${number}x${number}`; focalPoint: ArtFocalPoint; focalPoints?: Partial<Record<'dawn' | 'vigil', ArtFocalPoint>> }
...
export const artSpecs: Record<AssetId, ArtSpec> = {
  ...
  'I3-daily-ayah': { label: 'Daily ayah atmosphere', size: '1662x946', focalPoint: { x: 0.55, y: 0.52 }, focalPoints: { dawn: { x: 0.54, y: 0.48 }, vigil: { x: 0.56, y: 0.55 } } },
  ...
  'I10-journal-empty': { label: 'Journal empty state', size: '1536x1024', focalPoint: { x: 0.66, y: 0.67 }, focalPoints: { dawn: { x: 0.62, y: 0.64 }, vigil: { x: 0.7, y: 0.68 } } },
