...
export function RitualCard({ icon, title, subtitle, done, onPress }: { icon: keyof typeof Ionicons.glyphMap; title: string; subtitle: string; done: boolean; onPress: () => void }) {
  const t = useTheme();
  return <Pressable onPress={onPress} style={({ pressed }) => ({ backgroundColor: t.surface, borderWidth: 1, borderColor: done ? t.gold : t.border, borderRadius: radius.inner, padding: spacing.lg, flexDirection: 'row', alignItems: 'center', gap: spacing.md, opacity: pressed ? 0.78 : 1 })}>
