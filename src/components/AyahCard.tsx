...
export function AyahCard({ ayah, done, onComplete }: { ayah: DailyAyah; done?: boolean; onComplete?: () => void }) {
  ...
  return <ArtSlot id="I3-daily-ayah" variant="hero" height={280} radius={radius.hero}>
    <View style={{ minHeight: 280, padding: spacing.xl, justifyContent: 'space-between' }}>
      <View>
        ...
        <Text style={{ color: 'rgba(246,242,233,0.68)', fontFamily: fonts.sans, fontSize: 11, marginTop: spacing.md }}>{t('today.ayahAttribution')}</Text>
      </View>
      {onComplete ? <Pressable
        accessibilityRole="checkbox"
        accessibilityState={{ checked: Boolean(done) }}
        accessibilityLabel={t('today.ayahReadA11y', { reference: ayah.reference })}
