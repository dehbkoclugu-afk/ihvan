...
export default function QuranHistory() {
  ...
  return <Screen>
    ...
    <View style={{ flexDirection: direction, gap: spacing.sm, marginTop: spacing.xl }}>{[7, 30, 90].map((period) => {
      ...
      return <View key={period} style={{ flex: 1, minWidth: 0, backgroundColor: theme.surface, borderRadius: radius.inner, paddingVertical: spacing.lg, paddingHorizontal: spacing.xs, alignItems: 'center' }}><Text style={{ color: theme.gold, fontFamily: fonts.serif, fontSize: 24 }}>{formatLocaleNumber(summary.ayahs)}</Text><Text numberOfLines={1} adjustsFontSizeToFit style={{ width: '100%', color: theme.inkSoft, fontFamily: fonts.sansSemiBold, fontSize: 10, textAlign: 'center' }}>{t('history.period', { count: period })}</Text><Text numberOfLines={1} adjustsFontSizeToFit style={{ width: '100%', color: theme.inkFaint, fontFamily: fonts.sans, fontSize: 9, marginTop: 3, textAlign: 'center' }}>{t('history.activeDays', { count: summary.activeDays })}</Text></View>;
    })}</View>
    ...
    <View style={{ gap: spacing.xs, marginTop: spacing.md }}>{days.map((key, index) => {
      ...
      return <View key={key} style={{ flexDirection: direction, alignItems: 'center', gap: spacing.sm, minHeight: 44, paddingHorizontal: spacing.md, borderRadius: radius.inner, backgroundColor: index === 0 ? theme.goldSoft : theme.surface }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ width: 96, color: index === 0 ? theme.gold : theme.inkSoft, fontFamily: fonts.sansMedium, fontSize: 11, textAlign: align }}>{index === 0 ? t('history.today') : formatLocaleDate(dateFromKey(key), { weekday: 'short', day: 'numeric', month: 'short' })}</Text><View style={{ flex: 1, height: 5, borderRadius: 3, backgroundColor: theme.surfaceAlt }}><View style={{ width: `${(count / maxCount) * 100}%`, height: 5, borderRadius: 3, backgroundColor: theme.gold }} /></View><Text numberOfLines={1} adjustsFontSizeToFit style={{ width: 62, textAlign: locale === 'ar' ? 'left' : 'right', color: count ? theme.ink : theme.inkFaint, fontFamily: fonts.sansSemiBold, fontSize: 11 }}>{t('history.dayAyahs', { count })}</Text></View>;
    })}</View>
  </Screen>;
}
