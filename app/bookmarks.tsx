...
export default function Bookmarks() {
  ...
  return <Screen>
    ...
    {ayahs.length ? <View style={{ marginTop: spacing.lg, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: t.border, backgroundColor: t.surface, borderRadius: radius.inner, paddingHorizontal: spacing.md }}><Ionicons name="search-outline" size={18} color={t.inkFaint} /><TextInput accessibilityLabel="Yer imlerinde sûre veya ayet ara" value={query} onChangeText={setQuery} placeholder="Sûre adı veya 2:255" placeholderTextColor={t.inkFaint} autoCapitalize="none" style={{ flex: 1, color: t.ink, fontFamily: fonts.sans, paddingHorizontal: spacing.sm, paddingVertical: 12 }} />{query ? <Pressable accessibilityRole="button" accessibilityLabel="Aramayı temizle" onPress={() => setQuery('')}><Ionicons name="close-circle" size={18} color={t.inkFaint} /></Pressable> : null}</View> : null}

    {!ayahs.length ? <View style={{ backgroundColor: t.surface, borderRadius: radius.card, padding: spacing.xxl, alignItems: 'center', marginTop: spacing.xxl }}><Ionicons name="bookmark-outline" size={30} color={t.gold} /><Text style={{ color: t.ink, fontFamily: fonts.sansSemiBold, fontSize: 16, marginTop: spacing.md }}>Okurken saklamak istediğin ayetlere yer imi ekleyebilirsin.</Text><Pressable accessibilityRole="button" accessibilityLabel="Kur’an’a git" onPress={() => router.replace('/(tabs)/quran')} style={{ backgroundColor: t.gold, borderRadius: radius.pill, paddingHorizontal: spacing.xl, paddingVertical: 11, marginTop: spacing.lg }}><Text style={{ color: t.onGold, fontFamily: fonts.sansBold }}>Kur’an’a git</Text></Pressable></View> : !visibleAyahs.length ? <View style={{ backgroundColor: t.surface, borderRadius: radius.inner, padding: spacing.xl, alignItems: 'center', marginTop: spacing.lg }}><Ionicons name="search-outline" size={24} color={t.inkFaint} /><Text style={{ color: t.inkSoft, fontFamily: fonts.sansSemiBold, marginTop: spacing.sm }}>Bu aramada yer imi bulunamadı.</Text></View> : <View style={{ gap: spacing.md, marginTop: spacing.xl }}>{visibleAyahs.map((ayah) => {
      const surah = QURAN_SURAHS[ayah.surah - 1];
      ...
      return <View key={`${ayah.surah}:${ayah.ayah}`} style={{ backgroundColor: t.surface, borderWidth: 1, borderColor: t.border, borderRadius: radius.card, padding: spacing.lg }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}><Ionicons name="bookmark" size={17} color={t.gold} /><Text style={{ flex: 1, color: t.gold, fontFamily: fonts.sansSemiBold, fontSize: 12, marginLeft: spacing.sm }}>{reference}</Text><Pressable accessibilityRole="button" accessibilityLabel={`${reference} yer imini kaldır`} hitSlop={8} onPress={() => toggleBookmark(ayah.surah, ayah.ayah)} style={{ width: 36, height: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 18, backgroundColor: t.goldSoft }}><Ionicons name="trash-outline" size={17} color={t.gold} /></Pressable></View>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={`${reference} ayetini sûrede aç`} onPress={() => router.push({ pathname: '/surah/[id]', params: { id: `${ayah.surah}`, ayah: `${ayah.ayah}` } })} style={({ pressed }) => ({ flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', marginTop: spacing.md, opacity: pressed ? 0.65 : 1 })}><Text style={{ color: t.gold, fontFamily: fonts.sansSemiBold, fontSize: 12 }}>Sûrede aç</Text><Ionicons name="arrow-forward" size={15} color={t.gold} style={{ marginLeft: 5 }} /></Pressable>
      </View>;
    })}</View>}
