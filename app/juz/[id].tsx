...
export default function JuzDetail() {
  ...
  return <Screen>
    ...
    <View style={{ alignItems: 'center', marginTop: spacing.xl }}><Text style={{ color: theme.ink, fontFamily: fonts.serif, fontSize: 30, textAlign: 'center' }}>{t('quran.juzTitle', { number: juz.id })}</Text><Text style={{ color: theme.inkSoft, fontFamily: fonts.sans, marginTop: 5, textAlign: 'center' }}>{t('reader.juzMeta', { juz: juz.id, count: juz.ayahCount })}</Text></View>
    ...
    {ayahs.map((ayah, index) => {
      ...
      return <View key={key}>
        {startsSurah ? <View style={{ alignItems: 'center', paddingTop: spacing.xl, paddingBottom: spacing.sm }}><Text style={{ color: theme.ink, fontFamily: fonts.quran, fontSize: 28, writingDirection: 'rtl', textAlign: 'right' }}>{surah.arabicName}</Text><Text style={{ color: theme.inkSoft, fontFamily: fonts.sansSemiBold, marginTop: 4 }}>{surah.transliteration} · {surah.id}</Text></View> : null}
