...
export default function Quran() {
  ...
  return <Screen tabbed>
    ...
    <View style={{ backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, borderRadius: radius.card, padding: spacing.lg, marginTop: spacing.lg }}>
      ...
      <View accessibilityRole="radiogroup" style={{ flexDirection: direction, gap: spacing.sm, marginTop: spacing.md }}>{([5, 10, 20] as QuranReadingGoal[]).map((goal) => <Pressable key={goal} accessibilityRole="radio" accessibilityState={{ selected: readingGoal === goal }} onPress={() => setReadingGoal(goal)} style={{ flex: 1, borderRadius: radius.pill, minHeight: 44, alignItems: 'center', justifyContent: 'center', backgroundColor: readingGoal === goal ? theme.gold : theme.surfaceAlt }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: readingGoal === goal ? theme.onGold : theme.inkSoft, fontFamily: fonts.sansSemiBold, fontSize: 11 }}>{t('quran.goalOption', { count: goal })}</Text></Pressable>)}</View>
    </View>
    ...
    <View style={{ backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, borderRadius: radius.card, padding: spacing.lg, marginTop: spacing.lg }}>
      ...
      <View accessibilityRole="radiogroup" style={{ flexDirection: direction, gap: spacing.sm, marginTop: spacing.md }}>{QURAN_PLAN_DAYS.map((days) => <Pressable key={days} accessibilityRole="radio" accessibilityState={{ selected: quranPlanDays === days }} onPress={() => setQuranPlanDays(days)} style={{ flex: 1, minHeight: 44, borderRadius: radius.pill, alignItems: 'center', justifyContent: 'center', backgroundColor: quranPlanDays === days ? theme.gold : theme.surfaceAlt }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: quranPlanDays === days ? theme.onGold : theme.inkSoft, fontFamily: fonts.sansSemiBold, fontSize: 11 }}>{t('quran.hatimDays', { count: days })}</Text></Pressable>)}</View>
    </View>
    ...
    <View accessibilityRole="radiogroup" style={{ flexDirection: direction, backgroundColor: theme.surface, borderRadius: radius.pill, padding: 4, marginTop: spacing.xl }}>
      {([['surahs', t('quran.surahs')], ['juzs', t('quran.juzs')]] as const).map(([mode, label]) => <Pressable key={mode} accessibilityRole="radio" accessibilityState={{ selected: browseMode === mode }} onPress={() => { setBrowseMode(mode); if (mode === 'juzs') setQuery(''); }} style={{ flex: 1, minHeight: 44, borderRadius: radius.pill, alignItems: 'center', justifyContent: 'center', backgroundColor: browseMode === mode ? theme.gold : 'transparent' }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: browseMode === mode ? theme.onGold : theme.inkSoft, fontFamily: fonts.sansSemiBold }}>{label}</Text></Pressable>)}
    </View>
    <View accessibilityRole="radiogroup" style={{ flexDirection: direction, gap: spacing.sm, marginTop: spacing.sm }}>{([['all', t('quran.filterAll')], ['incomplete', t('quran.filterIncomplete')], ['complete', t('quran.filterComplete')]] as const).map(([filter, label]) => <Pressable key={filter} accessibilityRole="radio" accessibilityState={{ selected: progressFilter === filter }} onPress={() => setProgressFilter(filter)} style={{ flex: 1, minHeight: 44, alignItems: 'center', justifyContent: 'center', borderRadius: radius.pill, backgroundColor: progressFilter === filter ? theme.goldSoft : theme.surface }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: progressFilter === filter ? theme.gold : theme.inkSoft, fontFamily: fonts.sansSemiBold, fontSize: 10 }}>{label}</Text></Pressable>)}</View>
