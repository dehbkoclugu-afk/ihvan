...
export default function Worship() {
  ...
  const { notificationsEnabled, reminderMinutesBefore, setNotificationsEnabled, setReminderMinutesBefore } = usePrayerSettingsStore();
  ...
  useEffect(() => {
    ...
    void refreshPrayerNotifications(location.latitude, location.longitude, location.label, reminderMinutesBefore).then((ok) => {
      if (!ok) setNotificationsEnabled(false);
    }).catch(() => {});
  }, [location, notificationsEnabled, reminderMinutesBefore, setNotificationsEnabled]);
  ...
  async function toggleNotifications() {
    ...
    try {
      if (notificationsEnabled) {
        ...
        await enablePrayerNotifications(location.latitude, location.longitude, location.label, reminderMinutesBefore);
