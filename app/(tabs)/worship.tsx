...
export default function Worship() {
  ...
  useEffect(() => {
    ...
    if (!notificationPrayers.length) {
      ...
      void cancelPrayerNotifications().then(() => {
        ...
        setNotificationError('Seçili namaz kalmadığı için bildirimler kapatıldı; eski plan cihazdan kaldırılamadı. Daha sonra tekrar deneyebilirsin.');
        setNotificationsEnabled(false);
      });
    ...
    const timeout = setTimeout(() => {
      ...
      void refreshPrayerNotifications(location.latitude, location.longitude, location.label, reminderMinutesBefore, notificationPrayers).then((ok) => {
        if (notificationOperation.current !== operation) return;
        if (ok) {
          lastScheduledSignature.current = signature;
        } else {
          lastScheduledSignature.current = null;
          setNotificationsEnabled(false);
  ...
  return <Screen tabbed>
    ...
    {location ? <><PrayerTimesCard location={location} /><Pressable accessibilityRole="button" accessibilityLabel={`Vakit bildirimlerini ${notificationsEnabled ? 'kapat' : 'aç'}`} accessibilityState={{ disabled: notificationBusy }} disabled={notificationBusy} onPress={() => void toggleNotifications()} style={{ marginTop: spacing.sm, backgroundColor: t.surface, borderWidth: 1, borderColor: t.border, borderRadius: radius.inner, padding: spacing.md, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, opacity: notificationBusy ? 0.6 : 1 }}><Ionicons name={notificationsEnabled ? 'notifications' : 'notifications-outline'} size={19} color={notificationsEnabled ? t.gold : t.inkSoft} /><View style={{ flex: 1 }}><Text style={{ color: t.ink, fontFamily: fonts.sansSemiBold, fontSize: 13 }}>Vakit bildirimleri</Text><Text style={{ color: t.inkFaint, fontFamily: fonts.sans, fontSize: 10, marginTop: 2 }}>{notificationsEnabled ? `Açık · ${notificationPrayers.length} vakit · 10 gün planlandı` : 'Kapalı · yalnızca istersen açılır'}</Text></View>{notificationBusy ? <ActivityIndicator size="small" color={t.gold} /> : <Text style={{ color: t.gold, fontFamily: fonts.sansSemiBold, fontSize: 12 }}>{notificationsEnabled ? 'Kapat' : 'Aç'}</Text>}</Pressable><View style={{ marginTop: spacing.sm, backgroundColor: t.surface, borderWidth: 1, borderColor: t.border, borderRadius: radius.inner, padding: spacing.md, opacity: notificationBusy ? 0.65 : 1 }}><Text style={{ color: t.ink, fontFamily: fonts.sansSemiBold, fontSize: 12 }}>Bildirim namazları</Text><View style={{ flexDirection: 'row', gap: spacing.xs, marginTop: spacing.sm }}>{TRACKED_PRAYERS.map((prayer) => { const selected = notificationPrayers.includes(prayer.key); return <Pressable key={prayer.key} accessibilityRole="checkbox" accessibilityState={{ checked: selected, disabled: notificationBusy }} accessibilityLabel={`${prayer.label} vakti bildirimi`} disabled={notificationBusy} onPress={() => toggleNotificationPrayer(prayer.key)} style={{ flex: 1, borderRadius: radius.pill, paddingVertical: 8, alignItems: 'center', backgroundColor: selected ? t.gold : t.surfaceAlt }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: selected ? t.onGold : t.inkSoft, fontFamily: fonts.sansSemiBold, fontSize: 10 }}>{prayer.label}</Text></Pressable>; })}</View><Text style={{ color: t.inkFaint, fontFamily: fonts.sans, fontSize: 10, marginTop: spacing.sm }}>{notificationPrayers.length}/5 vakit seçili</Text></View><View style={{ marginTop: spacing.sm, backgroundColor: t.surface, borderWidth: 1, borderColor: t.border, borderRadius: radius.inner, padding: spacing.md, opacity: notificationBusy ? 0.65 : 1 }}><Text style={{ color: t.ink, fontFamily: fonts.sansSemiBold, fontSize: 12 }}>Bildirim zamanı</Text><View accessibilityRole="radiogroup" style={{ flexDirection: 'row', gap: spacing.xs, marginTop: spacing.sm }}>{([0, 5, 10, 15, 30] as PrayerReminderOffset[]).map((minutes) => <Pressable key={minutes} accessibilityRole="radio" accessibilityState={{ selected: reminderMinutesBefore === minutes, disabled: notificationBusy }} disabled={notificationBusy} onPress={() => setReminderMinutesBefore(minutes)} style={{ flex: 1, borderRadius: radius.pill, paddingVertical: 8, alignItems: 'center', backgroundColor: reminderMinutesBefore === minutes ? t.gold : t.surfaceAlt }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: reminderMinutesBefore === minutes ? t.onGold : t.inkSoft, fontFamily: fonts.sansSemiBold, fontSize: 10 }}>{minutes ? `${minutes} dk önce` : 'Tam vakit'}</Text></Pressable>)}</View></View>{notificationError ? <View accessibilityRole="alert" style={{ marginTop: spacing.sm, alignItems: 'flex-start' }}><Text style={{ color: t.danger, fontFamily: fonts.sans, fontSize: 11 }}>{notificationError}</Text>{notificationNeedsSettings ? <Pressable accessibilityRole="button" onPress={() => void Linking.openSettings().catch(() => setNotificationError('Bildirim ayarları açılamadı. Cihaz ayarlarından İhvan bildirimlerini açabilirsin.'))} style={{ marginTop: spacing.xs, paddingVertical: 5 }}><Text style={{ color: t.gold, fontFamily: fonts.sansSemiBold, fontSize: 11 }}>Bildirim ayarlarını aç</Text></Pressable> : null}</View> : null}<View style={{ marginTop: spacing.md }}><QiblaCard location={location} /></View></> : <View style={{ backgroundColor: t.surface, borderWidth: 1, borderColor: t.border, borderRadius: radius.card, padding: spacing.xl, alignItems: 'center' }}><View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: t.goldSoft, alignItems: 'center', justifyContent: 'center' }}><Ionicons name="navigate-outline" size={23} color={t.gold} /></View><Text style={{ color: t.ink, fontFamily: fonts.sansSemiBold, fontSize: 17, marginTop: spacing.md }}>Vakitleri ve kıbleyi aç</Text><Text style={{ color: t.inkSoft, fontFamily: fonts.sans, fontSize: 12, lineHeight: 18, textAlign: 'center', marginTop: spacing.sm }}>Hesaplama için yalnızca uygulamayı kullanırken mevcut konumun gerekir. Arka planda konum takibi yapılmaz.</Text><Pressable accessibilityRole="button" accessibilityLabel={locationNeedsSettings ? 'Konum ayarlarını aç' : 'Konumumu kullan'} accessibilityState={{ disabled: loading }} disabled={loading} onPress={() => void (locationNeedsSettings ? openLocationSettings() : requestLocation())} style={{ backgroundColor: t.gold, borderRadius: radius.pill, paddingHorizontal: spacing.xl, paddingVertical: 11, marginTop: spacing.lg, opacity: loading ? 0.6 : 1 }}>{loading ? <ActivityIndicator color={t.onGold} /> : <Text style={{ color: t.onGold, fontFamily: fonts.sansBold }}>{locationNeedsSettings ? 'Konum ayarlarını aç' : 'Konumumu kullan'}</Text>}</Pressable>{error ? <Text accessibilityRole="alert" style={{ color: t.danger, fontFamily: fonts.sans, fontSize: 11, textAlign: 'center', marginTop: spacing.sm }}>{error}</Text> : null}</View>}
