...
export default function Today() {
  ...
  const { count, doneSteps, doneDay, toggleStep } = useStreakStore();
  const today = dayKey();
  const done = (step: RitualStep) => doneDay === today && doneSteps.includes(step);
  ...
  return <Screen tabbed>
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      ...
      <View style={{ backgroundColor: t.surface, borderWidth: 1, borderColor: t.border, borderRadius: radius.inner, paddingHorizontal: 13, paddingVertical: 10, alignItems: 'center' }}><Text style={{ fontSize: 20 }}>🔥</Text><Text style={{ color: t.ink, fontFamily: fonts.sansBold, fontSize: 12 }}>{count} gün</Text></View>
    </View>
    <View style={{ marginTop: spacing.xl }}><AyahCard ayah={dailyAyah()} done={done('ayah')} onComplete={() => toggleStep('ayah')} /></View>
    <SectionHeader title="Bugünün ritmi" right={<Text style={{ color: t.inkSoft, fontFamily: fonts.sansMedium }}>{doneSteps.length}/4</Text>} />
