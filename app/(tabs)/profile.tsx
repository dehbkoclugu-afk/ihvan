...
export default function Profile() {
  ...
  return <Screen tabbed>
    ...
    <View accessibilityRole="radiogroup" style={{ flexDirection: rowDirection(locale), gap: spacing.sm, marginTop: spacing.md }}>{options.map((option) => <Pressable key={option.id} accessibilityRole="radio" accessibilityState={{ selected: pref === option.id }} onPress={() => setPref(option.id)} style={{ flex: 1, minWidth: 0, minHeight: 44, justifyContent: 'center', paddingHorizontal: spacing.xs, borderRadius: radius.pill, alignItems: 'center', backgroundColor: pref === option.id ? t.gold : t.surface }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: pref === option.id ? t.onGold : t.ink, fontFamily: fonts.sansSemiBold, fontSize: 12 }}>{option.label}</Text></Pressable>)}</View>
    ...
    <View accessibilityRole="radiogroup" style={{ flexDirection: rowDirection(locale), gap: spacing.sm, marginTop: spacing.md }}>{quranSizes.map((option) => <Pressable key={option.id} accessibilityRole="radio" accessibilityState={{ selected: quranTextSize === option.id }} onPress={() => setQuranTextSize(option.id)} style={{ flex: 1, minWidth: 0, minHeight: 44, justifyContent: 'center', paddingHorizontal: spacing.xs, borderRadius: radius.pill, alignItems: 'center', backgroundColor: quranTextSize === option.id ? t.gold : t.surface }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: quranTextSize === option.id ? t.onGold : t.ink, fontFamily: fonts.sansSemiBold, fontSize: 12 }}>{option.label}</Text></Pressable>)}</View>
    ...
    <View accessibilityRole="radiogroup" style={{ flexDirection: rowDirection(locale), gap: spacing.sm, marginTop: spacing.md }}>{quranMeals.map((option) => <Pressable key={option.id} accessibilityRole="radio" accessibilityState={{ selected: quranMeal === option.id }} onPress={() => setQuranMeal(option.id)} style={{ flex: 1, minWidth: 0, minHeight: 44, paddingHorizontal: spacing.xs, borderRadius: radius.pill, alignItems: 'center', justifyContent: 'center', backgroundColor: quranMeal === option.id ? t.gold : t.surface }}><Text numberOfLines={1} adjustsFontSizeToFit style={{ color: quranMeal === option.id ? t.onGold : t.ink, fontFamily: fonts.sansSemiBold, fontSize: 12 }}>{option.label}</Text></Pressable>)}</View>
